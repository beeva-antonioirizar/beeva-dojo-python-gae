...
def ks(villains, time, cost_distance_1000):
    ...
    for i, villain in enumerate(villains, start=1):
        for j in xrange(1, time + 1):
            if villain.life <= j:
                cost_travel = cost_distance_1000 * (villain.distance / 1000)
                if villain.reward - cost_travel + mem[i - 1][j - villain.life] >= mem[i - 1][j]:
                    mem[i][j] = villain.reward - cost_travel + mem[i - 1][j - villain.life]
                    grab[i][j] = 1
                else:
                    mem[i][j] = mem[i - 1][j]
            else:
